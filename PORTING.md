...
## Seam de testabilité process-exec + couverture unitaire étendue `[Claude]`

Hors plan de phases — à la demande explicite du sujet ("vérifier que chaque fonction Rust
a un test unitaire, y compris les fonctions impures"). Deux contraintes dures, confirmées
avant de commencer, pas prises sur la foi :

- `inquire` 0.9.4 garde son module `terminal`/`Backend` privé (`mod terminal;`, pas `pub
  mod`) — aucun moyen public d'injecter une saisie scriptée. Tout ce qui appelle
  `ui::select`/`ui::confirm`/`ui::text` (les wizards `cc`/`ch`/`cs`/`cdep`/`ddep`/`dc`/
  `dh`/`ds`/`provision`/l'étape `MultiSelect` de `bootstrap`) reste vérifié uniquement en
  direct via `tests/*.py`, sans changement.
- Ce qui restait réellement accessible : l'exécution de process (`ssh`/`sh`/`docker`),
  jusqu'ici verrouillée derrière des `Command::new(...).output()`/`.status()` directs.

**Le seam** (`src/ssh.rs`) : `pub(crate) fn spawn(program, args) -> io::Result<Output>` —
le seul point de passage de tout `Command::new("ssh"/"sh")` du fichier. Hors tests :
exactement `Command::new(program).args(args).output()`, aucun changement de comportement.
En test : un `thread_local!` optionnel intercepte l'appel ; `install_test_runner`/
`install_connected_test_runner` l'installent avec un `TestRunnerGuard` (RAII, nettoie au
`Drop` — nécessaire car `cargo test` réutilise les threads worker d'un test à l'autre) ;
`fake_output`/`with_fake_socket`/`FakeSocket` sont des utilitaires `#[cfg(test)]
pub(crate)` partagés (pas dupliqués par module). Thread-local plutôt qu'un paramètre
générique fileté partout : `run_with_master`/`exec_shell` sont appelés depuis ~15 sites
dans 5 fichiers, plusieurs eux-mêmes enveloppés par d'autres appelants — threader un
paramètre aurait fait onduler la signature de la moitié du crate. `docker.rs`'s
`run_docker_command` (branche localhost) et `backup.rs::exec_remote_shell` (collapsé pour
déléguer directement à `ssh::exec_shell` plutôt que dupliquer sa logique) réutilisent ce
même seam plutôt que d'en ouvrir un second.

**Piège découvert en cours de route** : `is_master_active` fait d'abord un
`socket.exists()` sur le vrai disque — une vérification filesystem, délibérément hors
seam — avant même d'appeler `spawn`. Un hôte de test fraîchement inventé n'a donc jamais
ce fichier, et `run_with_master` échoue avant d'atteindre le mock. Fixé avec
`with_fake_socket` : crée un fichier vide et inoffensif au chemin exact que
`control_socket` calculerait (pas un mock du filesystem, juste une fixture jetable sur le
vrai disque, nettoyée au `Drop`). Piège secondaire, découvert ensuite : un hôte de test
avec un `user@hostname:port` qui coïncide par hasard avec un vrai hôte de la flotte
(ex. réutiliser les vraies coordonnées de `mcm` "pour faire réaliste") fait écrire
`with_fake_socket` par-dessus un **vrai** socket ControlMaster actif — `std::fs::write`
dessus échoue avec `ENXIO` (`No such device or address`, l'erreur classique d'ouvrir un
socket UNIX comme un fichier régulier). Toujours utiliser des coordonnées manifestement
fictives (`test-host-unreachable.invalid`, etc.), jamais de vraies valeurs de
`hosts.yaml`. Piège tertiaire : deux tests utilisant le **même** hôte fictif avec
`with_fake_socket` en parallèle (`cargo test` exécute les tests sur plusieurs threads par
défaut) se marchent dessus — le `Drop` de l'un supprime le socket factice pendant que
l'autre le croit encore présent. Chaque test qui a besoin d'un socket factice utilise son
propre hôte fictif (un tag distinct dans le nom d'utilisateur/hostname), pas un helper
partagé entre plusieurs tests parallèles.

**Ce que ça couvre réellement**, au-delà du seam lui-même : `run_docker_command`
(routage localhost direct vs. remote quoté dans une commande `ssh` unique),
`do_generic_backup` (nettoyage best-effort + propagation de l'erreur d'origine sur un
échec `pg_dump` simulé, purge best-effort qui n'invalide pas un backup par ailleurs
réussi), `list_remote_backups`/`purge_old_backups` (filtrage + comptage d'appels contre
une sortie `ls -1t` simulée), `push_file`/`push_binary` de `provision.rs` (jusqu'ici sans
aucun test, ni unitaire ni live — chemin local vs. distant encodé en base64, découpage en
blocs, échec d'intégrité qui saute bien le `chmod +x`), `is_installed` de `bootstrap.rs`,
et `run_with_master`'s échappement `{{`/`}}` + chemin d'échec "Failed to establish master
connection" (jusque-là uniquement couverts par le test `#[ignore]`d qui, lui, reste —
c'est la seule preuve qu'une vraie authentification SSH/ControlMaster fonctionne de bout
en bout).

**Ce que ça ne couvre PAS**, à dire explicitement — "chaque fonction a un test unitaire"
reste faux au sens littéral : les wizards interactifs `cmd_*` restent live-only
(confirmé infaisable) ; les helpers filesystem de `ssh.rs` (`control_path`,
`materialize_identity_key`) restent live-only ; `build_release_binary`
(`backup_agent.rs`, compile un vrai `cargo build --release`) et l'envoi `curl` réel de
`notify_failure` restent délibérément live-only — un faux code de sortie n'y prouverait
rien de plus qu'une vérification live honnête. `sha256_hex` (`provision.rs`) reste réel
et non mocké dans les nouveaux tests de `push_binary` : déterministe, rapide, seul le
côté SSH autour est intercepté.

Vérifié : `cargo test` (121 tests, 1 `#[ignore]`d), `cargo clippy --all-targets -- -D
